...
export default function Login() {
  ...
  useEffect(() => {
    async function checkExistingUsers() {
      try {
        ...
        if (data === 0 || error) setNoUsersExist(true);
      } catch {}
    }
    checkExistingUsers();
  }, []);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    setErrorMsg('');
    if (!username || !password) { setErrorMsg('Enter both username and password.'); return; }
    setLoading(true);
    try {
  ...
  return (
    <div className="auth-root">
      ...
      <div className="auth-wrapper animate-scale-in">
        ...
        {noUsersExist && (
          <div className="auth-setup-hint animate-fade-in">
            <span>First time here?</span>
            <Link href="/register" className="setup-link">Set up your account →</Link>
          </div>
        )}
      </div>
